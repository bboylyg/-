...
conv_ = partial(layers.Conv2D, kernel_size=3, strides=1, padding='same', activation='relu')

def block(in_tensor, filters, n_convs):
    conv_block = in_tensor
    for _ in range(n_convs):
        conv_block = conv_(filters=filters)(conv_block)
    return conv_block

def vgg_(in_shape=(227, 227, 3), n_classes=1000, opt='sgd', n_stages_per_blocks=[2, 2, 3, 3, 3]):
...
def vgg16(in_shape=(227, 227, 3), n_classes=1000, opt='sgd'):
    return vgg_(in_shape, n_classes, opt)

def vgg19(in_shape=(227, 227, 3), n_classes=1000, opt='sgd'):
    return vgg_(in_shape, n_classes, opt, [2, 2, 4, 4, 4])

if __name__ == '__main__':
    model = vgg19()
    #plot_model(model, to_file="F://VGG_plot.png", show_shapes=True, show_layer_names=True)
    print(model.summary())
